% Script to integrate and save NORAD TLE initial state of the GLONASS satellites 
...
cspice_furnsh ( 'kernels\spk\TLE_GLONASS.bsp');
cspice_furnsh ( 'kernels\spk\2K25_GLONASS.bsp');
...
for k = 1:24
    ...
    [state, ltime] = cspice_spkezr (GLONASSID, et0,'J2000', 'LT+S', '399');
    [T,STATE] = orbit_plot(state,tspan,mu,plot_flag);
    
    if data == 1
        
         state = [T,STATE];
         writematrix(state,GLONASSID);
    end
    
end

%% We have constructed the spk for the GLONASS constellation up to 2025. let's validate graphically the SPK
...
for k = 1:24
    ...
    [state, ltime] = cspice_spkezr (GLONASSID, etspan,'J2000', 'NONE', '399');
