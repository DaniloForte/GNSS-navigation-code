% Script to integrate and save NORAD TLE initial state of the GPS satellites 
...
cspice_furnsh ( 'kernels\spk\TLE_GPS.bsp');
cspice_furnsh ( 'kernels\spk\2K25_GPS.bsp');
...
for k = 1:30
    ...
    [state, ltime] = cspice_spkezr (GPSID, et0,'J2000', 'NONE', '399');
    [T,STATE] = orbit_plot(state,tspan,mu,plot_flag);
    
    if data == 1
        
         state = [T,STATE];
         writematrix(state,GPSID);
    end
    
end

%% We have constructed the SPK for the GPS constellation up to 2025. let's validate graphically the SPK

if data == 1
    etspan = linspace(et0,etEND-10 ,10000);
else
    etspan = T';
end
...
for k = 1:30
    ...
    [state, ltime] = cspice_spkezr (GPSID, etspan,'J2000', 'NONE', '399');
